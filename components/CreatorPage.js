...
const CreatorPage = ({ username }) => {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      const [supportersData, campaignsData, userData] = await Promise.all([
        getUserSupporters(username),
      ...
      if (userData) {
        ...
        setCreator({
          name: userData.name,
          avatar: userData.avatar?.includes("http")
            ? userData.avatar
            : `/${userData.avatar}`,
          coverImage: userData.coverImage,
          bio: userData.bio,
          supporters: userData.supporters,
            ...
            <div className="relative bg-white p-6 rounded-xl shadow-lg border border-gray-200 md:flex md:items-end md:space-x-6">
              ...
              <div className="mt-4 md:mt-0 flex-shrink-0">
                <button
                  ...
                  className="w-full md:w-auto bg-amber-500 text-white font-bold py-3 px-8 rounded-full hover:bg-amber-600 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1 flex items-center justify-center gap-2"
                >
                ...
                {campaigns.length === 0 ? (
                  <div className="text-center py-16">
                    ...
                    <p className="text-gray-500 mt-2">
                      This creator has not launched any campaigns yet.
                    </p>
                  </div>
                ) : (
              ...
              <aside className="lg:w-1/3 mt-8 lg:mt-0">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                  ...
                  {supporters.length === 0 ? (
                    ...
                    <ul className="space-y-4">
                      {supporters
                        .filter((user) => user.to_user === creator.username)
                        .sort((a, b) => a.amount > b.amount)
                        .map((currUser) => (
                          <li
                            key={currUser.order_id}
                            className="border-b border-gray-200 pb-4 flex"
                          >
                            <span className="text-gray-800">
                              From{" "}
                              <strong>
                                {currUser.name} - ₹{currUser.amount} &nbsp;
                              </strong>{" "}
                              <br /> <em>"{currUser.message}"</em>
                            </span>
                          </li>
                        ))}
                    </ul>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mt-6">
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      <Twitter size={18} className="mr-3" />@{creator.username}
                    </a>
                    <a
                      href="#"
